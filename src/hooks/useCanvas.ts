...
export const useInteractiveCanvas = (
  ...
  const handlePointerMove = (e: MouseEvent | TouchEvent) => {
    ...
    if (isDragging && canvasRef.current && img) {
      ...
      const updateDx =
        canvasVals.current.dx +
        (event.clientX - prevDragPosition.x) *
          Math.min(window.devicePixelRatio, 2);
      const updateDy =
        canvasVals.current.dy +
        (event.clientY - prevDragPosition.y) *
          Math.min(window.devicePixelRatio, 2);

      // Update image position (constrain to edges of image)
      if (
        updateDx <= 0 &&
        updateDx >= canvas.offsetWidth - canvasVals.current.dw
      ) {
        canvasVals.current.dx = updateDx;
      }
      if (
        updateDy <= 0 &&
        updateDy >= canvas.offsetHeight - canvasVals.current.dh
      ) {
